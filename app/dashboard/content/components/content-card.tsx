...
export function ContentCard({ content, typeLabel }: { content: Content; typeLabel: string }) {
  const status: "pending" | "completed" | "failed" =
    content.status ?? (content.contentError ? "failed" : content.output || content.generatedContent ? "completed" : "pending");
  const preview = content.generatedContent || content.output || content.prompt;
  return (
    <Card>
      <div className="flex items-center justify-between">
        <div className="min-w-0">
          <div className="truncate text-sm font-medium">{content.title || "Untitled"}</div>
        ...
        <div className="text-xs text-zinc-500 dark:text-zinc-400">{formatDateTime(content.createdAt)}</div>
      </div>
      {preview ? (
        <div className="mt-3 text-xs text-zinc-600 line-clamp-4 dark:text-zinc-400">{preview}</div>
      ) : null}
